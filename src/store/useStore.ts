...
async function doWrite(tab: TabState, path: string): Promise<number> {
...
export const useStore = create<Store>((set, get) => ({
  ...
  init: async () => {
    ...
    if (state && Array.isArray(state.tabs) && state.tabs.length) {
      ...
      const activeTabId = state.activeTabId && state.tabs.some((t) => t.id === state.activeTabId)
        ? state.activeTabId
        : state.tabs[0].id;
      set({
        tabs: state.tabs,
  ...
  doCloseTabs: (ids) => {
    ...
    set((s) => {
      ...
      return { tabs, activeTabId, findOpen: false, replaceOpen: false };
    });
  },
  ...
  resolveConfirm: async (choice) => {
    ...
    if (choice === "save") {
      ...
      if (!ok) return; // a save was cancelled, abort
    }
  ...
  saveTab: async (id) => {
    ...
    try {
      const size = await doWrite(tab, tab.filePath);
      get().updateTab(id, { dirty: false, size });
      return true;
    } catch (e) {
      await api.nativeMessage("Jotpad", String(e));
      return false;
  ...
  saveAsTab: async (id) => {
    ...
    try {
      const size = await doWrite(tab, path);
      get().updateTab(id, {
        filePath: path,
        title: basename(path),
        dirty: false,
